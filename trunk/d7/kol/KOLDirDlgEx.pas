...
{$DEFINE ANSI_DIRS}
           { force using standard ANSI API only any way. Less code but slower
             about twice. But if ANSI_DIRS is NOT defined, therefore ANSI API
             is used in Win9x system anyway (since these do not support UNICODE). }
...
type
  ...
  {$ELSE}
  TFindFirstFileEx = function(lpFileName: PWideChar; fInfoLevelId: TFindexInfoLevels;
    lpFindFileData: Pointer; fSearchOp: TFindexSearchOps; lpSearchFilter: Pointer;
    dwAdditionalFlags: DWORD): THandle; stdcall;
  {$ENDIF}
  ...
  protected
    ...
    {$IFNDEF ANSI_DIRS}
    FFindFirstFileExA: TFindFirstFileExA;
    {$ENDIF}
    k32: THandle;
...
begin
  ...
  Bar := NewToolbar( Pn, caClient, [
    ...
    Bmp.ReleaseHandle,
    [ PChar( {$IFDEF TOOLBAR_DOT_NOAUTOSIZE_BUTTON} '.' + {$ENDIF} C ) ], [ 0 ] );
  ...
  Bar.CustomData := Buffer;
  {$ENDIF USE_GRUSH}
  ToolbarSetTooltips( Bar, i, [ PChar( T ) ] );
  Bmp.Free;
end;
...
var HasSubDirs: Boolean;
    ...
    {$ELSE}
    Find32: TWin32FindDataW;
    Find32A: TWin32FindData;
    {$ENDIF}
    ii, n: Integer;
begin
  HasSubDirs := FALSE;
  ...
  begin
    ...
    begin
      ...
      {$ELSE}
      if ForceAnsi then
        F := FFindFirstFileExA( PChar( DirTree.TVItemPath( node, '\' ) + '\*.*' ),
          FindExInfoStandard, @ Find32A, FindExSearchLimitToDirectories, nil, 0 )
      else
        F := FFindFirstFileEx( PWideChar( WideString( DirTree.TVItemPath( node, '\' ) + '\*.*' ) ),
          FindExInfoStandard, @ Find32, FindExSearchLimitToDirectories, nil, 0 );
      {$ENDIF}
      ...
      begin
        ...
        begin
          ...
          if ForceAnsi then
          begin
            if not FindNextFile( F, Find32A ) then break;
          end else
          if not FindNextFileW( F, Find32 ) then break;
          {$ENDIF}
        end;
    ...
    begin
      {$IFDEF ANSI_DIRS}
      F := FindFirstFile( PChar( DirTree.TVItemPath( node, '\' ) + '\*.*' ), Find32 );
      {$ELSE}
      if ForceAnsi then
        F := FindFirstFile( PChar( DirTree.TVItemPath( node, '\' ) + '\*.*' ), Find32A )
      else
        F := FindFirstFileW( PWideChar( WideString( DirTree.TVItemPath( node, '\' ) + '\*.*' ) ), Find32 );
      {$ENDIF}
      if F <> INVALID_HANDLE_VALUE then
      begin
        while TRUE do
...
begin
  ...
  begin
    ...
    {$ELSE}
    if ForceAnsi then
      FFindFirstFileExA := GetProcAddress( k32, 'FindFirstFileExA' )
    else
      FFindFirstFileEx := GetProcAddress( k32, 'FindFirstFileExW' );
    {$ENDIF}
  end;
  ...
  begin
    ...
    if StrEq( CurPath, s ) then
      Form.ModalResult := 1
           ...
           Path := s;
           DirTree.TVExpand( DirTree.TVSelected, TVE_EXPAND );
         end;
  end;
end;
  ...
  TRY
    ...
    TRY
      ...
      begin
        ...
        {$ELSE}
        if ForceAnsi then
          F := FFindFirstFileExA( PChar( String( p + '*.*' ) ), FindExInfoStandard, @ Find32A,
            FindExSearchLimitToDirectories, nil, 0 )
        else
          F := FFindFirstFileEx( PWideChar( p + '*.*' ), FindExInfoStandard, @ Find32,
            FindExSearchLimitToDirectories, nil, 0 );
        {$ENDIF}
        if F <> INVALID_HANDLE_VALUE then
        begin
          TRY
            ...
            begin
              ...
              if ForceAnsi then
              begin
                if Find32A.dwFileAttributes and FILE_ATTRIBUTE_DIRECTORY <> 0 then
                if (Find32A.cFileName <> String( '.' )) and (Find32A.cFileName <> '..') then
                if DoFilterAttrs( Find32A.dwFileAttributes, Find32A.cAlternateFileName ) then
                  SLA.Add( Find32A.cFileName );
                if not FindNextFile( F, Find32A ) then break;
              end
                else
              begin
                if Find32.dwFileAttributes and FILE_ATTRIBUTE_DIRECTORY <> 0 then
                if (Find32.cFileName <> WideString( '.' )) and (Find32.cFileName <> '..') then
                if DoFilterAttrs( Find32.dwFileAttributes, Find32.cAlternateFileName ) then
                  SL.Add( Find32.cFileName );
                if not FindNextFileW( F, Find32 ) then break;
              end;
            ...
            SL.Sort( FALSE );
            {$ELSE}
            if ForceAnsi then SLA.Sort( FALSE )
                         else SL.Sort( FALSE );
            {$ENDIF}
          FINALLY
            FindClose( F );
        ...
        TRY
          ...
          for i := 0 to DL.Count-1 do
            ...
            begin
              ...
              SL.Add( DL.Names[ i ] );
              {$ELSE}
              if ForceAnsi then SLA.Add( DL.Names[ i ] )
                           else SL.Add( DL.Names[ i ] );
              {$ENDIF}
            end;
        FINALLY
          DL.Free;
  ...
  TRY
    ...
    begin
      ...
      begin
        ...
        begin
          if not IntIn( GetDriveType( PChar( n + '\' ) ),
...
begin
  ...
  Ico := NewIcon;
  Ico.Size := LinksImgList.ImgWidth;
  Ico.SetHandleEx( LinksImgList.ExtractIcon( FileIconSystemIdx( s ) ) );
